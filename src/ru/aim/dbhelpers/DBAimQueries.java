...
public class DBAimQueries {
	/**
	 * Get the query for getting actual User's aims, actual, i.e. WebAim's
	 * attribute 'archive'=0
	 * 
	...
	public static String getUserAimsByDeadline(int userId) {
		return String.format(
				"select * from user_aim " + "where users_id = '%s' AND archive = 0 " + "order by deadline ASC", userId);
	}
	...
	public static String getWebUserByUsernameAndPassword(String userName, String password) {
		...
		return String.format("SELECT * FROM users " + "where login = '%s' " + "and password = '%s' ", userName,
				password);
	}
	...
	public static String getWebUserByUsername(String userName) {
		...
		return String.format("SELECT * FROM users " + "where login = '%s' ", userName);
	...
	public static String addWebUserByUserNameAndPassword(String userName, String password) {
		...
		return String.format("INSERT INTO users (login, password) " + "VALUES ('%s', '%s') ", userName, password);
	}
	...
	public static String getWebAimByAimName(int userId, String aimName) {
		...
		return String.format("SELECT * FROM user_aim " + "where users_id = '%d' " + "and name = '%s' ", userId,
				aimName);

	}

	/**
	 * According to has an aim date type or not, creates query to insert data:
	...
	public static String insertAim(WebAim aim, String deadline_tmp) {
		...
		if (deadline_tmp.equals("")) {
			return String.format(
					"INSERT INTO user_aim " + "(users_id, name, description, control) "
							+ "VALUES ('%d','%s', '%s', '%s')",
					aim.getUserId(), aim.getName(), aim.getDescription(), aim.getControl());

		} else
			return String.format(
					"INSERT INTO user_aim " + "(users_id, name, description, control, deadline) "
							+ "VALUES ('%d','%s', '%s', '%s', '%s')",
					aim.getUserId(), aim.getName(), aim.getDescription(), aim.getControl(), aim.getDeadline());
	}
	...
	public static String deleteAim(int userId, String nameOfAim, String description) {
		...
		return String.format("DELETE FROM user_aim " + "WHERE users_id='%d'AND name='%s' AND description='%s' ", userId,
				nameOfAim, description);
	}
	...
	public static String markAsArchived(int userId, String nameOfAim, String description) {
		...
		return String.format(
				"UPDATE user_aim SET archive=1 " + "WHERE users_id='%d'AND name='%s' AND description='%s' ", userId,
				nameOfAim, description);
	}
	...
	public static String markAsComplited(int userId, String nameOfAim, String description) {
		...
		return String.format("UPDATE user_aim SET result=1 " + "WHERE users_id='%d'AND name='%s' AND description='%s' ",
				userId, nameOfAim, description);
	}

	/**
	 * Get the query for getting archive User's aims, archive, i.e. WebAim's
	 * attribute 'archive'=1
	 * 
	 * @param userId
	 * @return String
	...
	public static String getArchiveUserAimsByDeadline(int userId) {
		return String.format(
				"SELECT * FROM user_aim " + "WHERE users_id = '%s' AND archive = 1 " + "ORDER BY deadline ASC", userId);
	}
	...
	public static String insertGroup(WebGroup group) {
		...
		return String.format(
				"INSERT INTO user_group " + "(users_id, name_group, password, creator) "
						+ "VALUES ('%d','%s', '%s', '%d')",
				group.getUserId(), group.getName(), group.getPassword(), creator);
	...
	public static String getWebGroupByName(String name) {
		...
		return String.format("SELECT * FROM user_group " + "WHERE name_group = '%s' AND creator=1 ", name);
	}
	...
	public static String getWebGroupByUserIdAndGroup(int userId, String nameOfGroup) {
		return String.format("SELECT * FROM user_group " + "WHERE users_id = '%d' AND name_group = '%s' ", userId,
				nameOfGroup);
	}
	...
	public static String getGroupsByUserId(int userId) {
		return String.format("SELECT * FROM user_group " + "WHERE users_id = '%d' ", userId);
	}
	...
	public static String insertAimGroup(WebAimGroup aimGroup) {
		...
		if (!(aimGroup.getGroupId() == 0)) {
			...
			return String.format("INSERT INTO aim_group " + "(user_aim_id, user_group_id) " + "VALUES ('%d','%d')",
					aimGroup.getAimId(), aimGroup.getGroupId());
		}
		return null;
	...
	/**
	 * Returns all information from User,Aim,Group by group name by deadline
	 * 
	 * @param groupName
	 * @param groupId
	 * @return String
	...
	public static String selectCollectorForGroup(String groupName) {
		...
		return String.format("SELECT * FROM user_group g " + "INNER JOIN aim_group ag ON g.id=ag.user_group_id "
				+ "INNER JOIN user_aim a ON ag.user_aim_id=a.id " + "INNER JOIN users u ON u.id=a.users_id "
				+ "WHERE g.name_group='%s' ORDER BY a.deadline", groupName);
	...
	public static String signoutFromGroup(int userId, int groupId) {
		return String.format("DELETE FROM user_group " + "WHERE users_id='%d'AND id='%d' AND creator=0", userId,
				groupId);
	}
	...
	public static String deleteGroup(String groupName) {
		return String.format("DELETE FROM user_group " + "WHERE id>0 AND name_group='%s' ", groupName);
	}

	/**
	 * Returns all information from User,Aim,Group by group name by rate
	 * 
	 * @param groupName
	 * @return
	 */
	public static String selectCollectorForGroupByRate(String groupName) {
		return String.format("SELECT * FROM users u " + "INNER JOIN user_group g ON u.id=g.users_id "
				+ "WHERE g.name_group='%s' ORDER BY u.solved DESC", groupName);
	}
